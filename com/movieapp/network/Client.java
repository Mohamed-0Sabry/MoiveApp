...
public class Client {
    ...
    private ExecutorService systemAudioExecutor;
     ...
     *
     * @param discoveryPort the UDP port on which the server is listening for
     *                      discovery requests
    ...
    public void startSystemAudioStreaming() {
        try {
            ...
            if (systemAudioExecutor == null || systemAudioExecutor.isShutdown()) {
                systemAudioExecutor = Executors.newSingleThreadExecutor();
            }
    
            systemAudioExecutor.submit(() -> {
                AudioStreamUtils.startSystemAudioStream(pcmBytes -> {
                    try {
                        if (audioSocket != null && !audioSocket.isClosed()) {
                            AudioPacket pkt = new AudioPacket(pcmBytes, clientUsername + "_system");
                        ...
                        System.err.println("[Client] Error sending system audio: " + e.getMessage());
                    }
                });
                isSystemAudioStreaming = true;
            });
        } catch (IOException e) {
            System.err.println("[Client] Error starting system audio streaming: " + e.getMessage());
        }
    }

    
    public void stopSystemAudioStreaming() {
        AudioStreamUtils.stopSystemAudioStream();
        isSystemAudioStreaming = false;
        
        if (systemAudioExecutor != null && !systemAudioExecutor.isShutdown()) {
            systemAudioExecutor.shutdownNow();
            systemAudioExecutor = null;
        }
    }
        ...
        audioExecutor.submit(() -> {
            ...
            while (isConnected && audioSocket != null && !audioSocket.isClosed()) {
                try {
                    ...
                    if (!isConnected || audioSocket == null || audioSocket.isClosed())
                        break;
